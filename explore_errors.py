...
def read_class_log():
    df = (pd.read_csv("outputs/diffliabs/liab_class.log",
...
def read_errors(reports):
    df = (pd.read_csv("outputs/diffliabs/liab_custom_lshe.csv")
            .set_index("adsh")
            .sort_index()
            .merge(reports, how='left', left_index=True, right_index=True))
...
algs = ["lshe_adv", "lcpc_old", "lcpc_new", "lcpc_m_old", "lcpc_m_new"]
