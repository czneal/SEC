...
class income_st_builder:
    ...
    known_errors = []
    potential_parents = [] 
    model_statement_classification = []
    model_sign_classification = []
    is_ready=True
    error_message=''
    ...
    def load_models(self,data_path):     
        ...
        try:
            with open (data_path+'\income_st_data.txt', 'rb') as fp:
        ...
        try:
            self.model_statement_classification = load_model(data_path+'\income_st_model0.h5')  
            #self.models_dictionary.update({0: model_statement_classification})
            self.model_sign_classification = load_model(data_path+'\income_st_model1.h5') 
    ...
    def al_build_tree(self,all_tags):
        trees_out,nodes_out = self.build_tree(all_tags,self.model_statement_classification,self.tag_to_code,self.rules,18,64)
        return(trees_out,nodes_out)
        
    def al_add_weights(self,trees):
        trees_out = self.append_weights(trees,self.model_sign_classification,self.tag_to_code,self.rules,15,64)
        return(trees_out)
    ...
    def build_tree(self,all_tags,class_model,tag_to_code,rules,max_len,batch_size):
        if 'NetIncomeLoss' not in all_tags:
            all_tags.append('NetIncomeLoss')   
    
        reconstructed = self.reconstruct(all_tags,class_model,tag_to_code,rules,max_len,batch_size)
        pairs_constructed = []
        ...
        for child in all_childs:
            all_parents = list(set([x[1] for x in reconstructed if x[1]!='' and x[0]==child and x[2]==1]))  
            if len(all_parents)>1:
                #у листа 2 и более родителя
                best_parent_det,selected_parents = self.get_best_parent_v2(child,all_parents,class_model,tag_to_code,rules,max_len,batch_size)
        ...
        for node in nodes_left:
            ...
            if len(tree)>0:
                ...
                search_for_NIL = [x for x in tree if x[0]=='NetIncomeLoss' or x[1]=='NetIncomeLoss']
                if len(search_for_NIL)>0:
                    tree_with_NIL_index=len(trees_to_analyse)-1
            else:
                nodes_out.append(node)
        #### теперь имеем список деревьев, номер дерева, в котором оказался NetIncomeLoss
        if tree_with_NIL_index<0:
            ## нет деревьев, привязанных к NetIncomeLoss
            for tree in trees_to_analyse:
                trees_out.append(tree[1])
        else:
            ##  берем дерево с NetIncomeLoss и пытаемся к нему подсоединить остальные
            ...
            for i in range(len(trees_to_analyse)):
                if i!=tree_with_NIL_index:
                    NIL_tags = self.prepare_data(trees_to_analyse[tree_with_NIL_index][1])
                    NIL_tags.append(trees_to_analyse[i][0])
                    reconstructed_subs = self.reconstruct(NIL_tags,class_model,tag_to_code,rules,max_len,batch_size)
                    sub_tree_parents = list(set([x[1] for x in reconstructed_subs if x[1]!='' and x[0]==trees_to_analyse[i][0] and x[2]==1]))  
                    ...
                    if len(sub_tree_parents)==0:
                        ...
                        united_tree.append((trees_to_analyse[i][0],sub_tree_parents[0]))
            trees_out[0] = united_tree
        return(trees_out,nodes_out)        
        
    #dependencies get_predictions_weight
    #returns list of trees with format child,parent,weight    
    def append_weights(self,trees,class_model,tag_to_code,rules,max_len,batch_size):
        trees_out=[]
        for tree in trees:
            predicted = self.get_predictions_weight(tree,class_model,tag_to_code,rules,max_len,batch_size)
            predicted_int=[]
            for i in range(predicted.shape[0]):
                if predicted[i]>=0.5:
                    ...
                    predicted_int.append(-1)
            trees_out.append(list(zip([x[0] for x in tree],[x[1] for x in tree],predicted_int)))
        return(trees_out)
        
    #dependencies parse_tag_without_y, sequence.pad_sequences
    #returns array of predictions           
    def get_predictions_weight(self,tree,class_model,tag_to_code,rules,max_len,batch_size):
        tags_childs=[]
        tags_parents=[]
        for pair in tree:
        ...
        predicted = class_model.predict([x_childs_fake,x_parents_fake], batch_size=batch_size)
        return(predicted[0:len(tags_parents)])            
        
        
    #dependencies reconstruct
    #returns best_parent (string), potential parents determined by network (list of strings)     
    def get_best_parent_v2(self,child,potential_parents,class_model,tag_to_code,rules,max_len,batch_size):
        all_tags = []
        all_tags.append(child)
        all_tags=all_tags+potential_parents
        ##child,parent,class,states[context,result]
        reconstructed = self.reconstruct(all_tags,class_model,tag_to_code,rules,max_len,batch_size)
        selected_parents = [(x[1],x[3]) for x in reconstructed if x[0]==child and x[2]==1]
        max_index=-1
        max_probablity=0
        best_parent=''
    ...
    def reconstruct(self,all_tags,class_model,tag_to_code,rules,max_len,batch_size):
        reconstructed = []
        combs = self.get_combinations(len(all_tags))
        triples = []
        for elem in combs:
            triples.append((all_tags[elem[0]],all_tags[elem[1]],all_tags[elem[2]]))
        tags_parents=[x[0] for x in triples]
        tags_childs=[x[1] for x in triples]
        tags_contexts=[x[2] for x in triples]
        predicted = self.get_predictions(tags_parents,tags_childs,tags_contexts,class_model,tag_to_code,rules,max_len,batch_size)
        ...
        for elem in all_tags:
            ...
            for parent in cur_parents:
                cur_states = [(x[1],x[2][0]) for x in cur_sets if x[0]==parent]            
    ...
    def get_predictions(self,tags_parents,tags_childs,tags_contexts,class_model,tag_to_code,rules,max_len,batch_size):
        x_parents = self.parse_tag_without_y(tags_parents,tag_to_code,rules)  
        x_childs = self.parse_tag_without_y(tags_childs,tag_to_code,rules) 
        x_contexts = self.parse_tag_without_y(tags_contexts,tag_to_code,rules) 
        x_parents = sequence.pad_sequences(x_parents, maxlen=max_len)
        x_childs = sequence.pad_sequences(x_childs, maxlen=max_len)
        ...
        predicted = class_model.predict([x_parents_fake,x_childs_fake,x_contexts_fake], batch_size=batch_size)
        return(predicted[0:len(tags_parents)])  
